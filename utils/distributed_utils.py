...
def test_dist_init(launcher, backend='nccl',port=10086, **kwargs):
    if mp.get_start_method(allow_none=True) is None:
        mp.set_start_method('spawn')
    if launcher == 'pytorch':
        _init_dist_pytorch(backend, **kwargs)
    elif launcher == 'mpi':
        _init_dist_mpi(backend, port=port,**kwargs)
    elif launcher == 'slurm':
        _init_dist_slurm(backend,  port=port,**kwargs)
    else:
        raise ValueError('Invalid launcher type: {}'.format(launcher))
...
class DistributedSequentialSampler(Sampler):
    def __init__(self, dataset, world_size=None, rank=None):
        ...
        #self.end = min(self.beg+sub_num, len(self.datasets))
